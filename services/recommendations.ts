...
export async function getPersonalizedRecommendations(
  ...
  try {
    ...
    void recordPerfMetric('recommendations_personalized', Date.now() - startedAt, true);
    return recommendations;
  } catch (error) {
    console.error('Failed to get personalized recommendations:', error);
    void recordPerfMetric('recommendations_personalized', Date.now() - startedAt, false);
    return [];
  }
}
...
export async function getCollaborativeRecommendations(
  ...
  try {
    ...
    void recordPerfMetric('recommendations_collaborative', Date.now() - startedAt, true);
    return recommendations;
  } catch (error) {
    console.error('Failed to get collaborative recommendations:', error);
    void recordPerfMetric('recommendations_collaborative', Date.now() - startedAt, false);
    return [];
  }
}
...
export async function learnUserPreferences(userId: string): Promise<UserPreferences> {
  try {
    ...
    checkins.forEach(checkin => {
      ...
      const timestamp = getCheckinDate(checkin);
      if (!timestamp) return;
    ...
    console.error('Failed to learn user preferences:', error);
    return getDefaultPreferences(userId);
  }
}
...
async function getSpotTimePatterns(placeId: string): Promise<TimePattern[]> {
  try {
    ...
    checkinsSnapshot.forEach((doc: any) => {
      const data = doc.data();
      const timestamp = getCheckinDate(data);
      if (!timestamp) return;
